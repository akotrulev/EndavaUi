...
public enum BrowserTypeEnumDriverManager {
    CHROME(
            ...
            (WebDriverManager) -> io.github.bonigarcia.wdm.WebDriverManager.chromedriver().setup()
    ),
    FIREFOX(
            Browser.FIREFOX.browserName(),
            FirefoxDriver::new,
            (WebDriverManager) -> io.github.bonigarcia.wdm.WebDriverManager.firefoxdriver().setup()
    ),
    EDGE(
            Browser.EDGE.browserName(),
            EdgeDriver::new,
            (WebDriverManager) -> io.github.bonigarcia.wdm.WebDriverManager.edgedriver().setup()
    ),
    SAFARI(
            Browser.SAFARI.browserName(),
            SafariDriver::new,
            (WebDriverManager) -> io.github.bonigarcia.wdm.WebDriverManager.chromiumdriver().setup()
    );

    BrowserTypeEnumDriverManager(
            String browserType,
            Supplier<RemoteWebDriver> remoteWebDriver,
            Consumer<WebDriverManager> webDriverManagerConsumer
    ) {
